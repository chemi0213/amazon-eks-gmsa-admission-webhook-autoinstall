...
sed -i 's/docker build . -t \${DOCKER_IMAGE}/docker build . -t \${DOCKER_IMAGE} --network=host/g' Makefile
sed -i "s/RUN go mod download/RUN export GOPROXY='direct' \&\& go mod download/g" Dockerfile
...
sed -i "s/  - example.com\/foo/  - ${ESCAPED_SIGNER_NAME}/g" config/e2e/rbac.yaml
...
sed -i "s/signerName: kubernetes.io\/kubelet-serving/signerName: $ESCAPED_SIGNER_NAME/g" create-signed-cert.sh
...
sed -i "s/.*CA_BUNDLE=.*/        CA_BUNDLE=$CA \\\/g" deploy-gmsa-webhook.sh

#FIXING FILE FOR MACOS USERS
MACOS=$(sw_vers)
if [[ $MACOS ]]
then
    sed -i "s/-w 0/-b 0/g" deploy-gmsa-webhook.sh
    sed -i "s/-w 0/-b 0/g" create-signed-cert.sh
fi

#RUNNING THE INSTALLATION
./deploy-gmsa-webhook.sh --file ./gmsa-manifests --image gcr.io/k8s-staging-gmsa-webhook/k8s-gmsa-webhook:latest --overwrite
